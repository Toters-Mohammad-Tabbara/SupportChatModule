...
interface MessagesDao {
    ...
    fun getMessagesSorted(conversationSid: String): PagingSource<Int, MessageDataItem>

    // Get last message
    @Query("SELECT * FROM message_table WHERE conversationSid = :conversationSid AND sendStatus = 3")
    fun getUnSentMessages(conversationSid: String): Flow<List<MessageDataItem>>

    // Get last message
    @Query("SELECT * FROM message_table WHERE sendStatus = 1")
